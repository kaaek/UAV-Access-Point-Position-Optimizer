function [uav_pos, baseline_br, sumlink_mbps] = randomSol(user_pos, M, N, AREA, H_M, H, F, P_T, P_N, BW, Rmin)
side = ceil(sqrt(AREA));
lb = repelem([0;0], N);
...
objective_fn = @(br) -sum(log10(br)); % proportional fairness
obj = @(x) objective_fn(br(x));

for i = 1:10  % 10 random starts
    x0 = lb + (ub - lb).*rand(size(lb));  % random initial point
    [x, fval] = fmincon(obj, x0, [], [], [], [], lb, ub, @(x) nonlcon(x,user_pos, H_M, H, F, P_T, P_N, BW, Rmin), opts);
    if fval < best_f
        best_f = fval;
        best_x = x;
    end
